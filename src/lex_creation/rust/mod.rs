...
use write::{
    write_action_rust, write_header_rust, write_is_exclusive_state_rust,
...
impl RustBackend {
    pub fn new() -> Self { RustBackend }
}

impl CodegenBackend for RustBackend {
    fn write_header(&self, definitions: &[Definition], mode: YytextMode, out: &mut dyn std::io::Write) -> std::io::Result<()> {
        write_header_rust(definitions, mode, out)
    }
    fn write_yytext_section(&self, mode: YytextMode, out: &mut dyn std::io::Write) -> std::io::Result<()> {
    ...
    fn write_is_exclusive_state(&self, file_parts: &FilePart, out: &mut dyn std::io::Write) -> std::io::Result<()> {
        write_is_exclusive_state_rust(file_parts, out)
    }
    fn write_action(&self, file_parts: &FilePart, out: &mut dyn std::io::Write) -> std::io::Result<()> {
    ...
    fn write_yylex(&self, in_yylex: &[String], mode: YytextMode, out: &mut dyn std::io::Write) -> std::io::Result<()> {
        write_yylex_rust(in_yylex, mode, out)
    }
    fn write_user_routine(&self, user_routine: &str, out: &mut dyn std::io::Write) -> std::io::Result<()> {
        write_user_routine_rust(user_routine, out)
    }
    fn output_filename(&self) -> &str { "ft_lex.yy.rs" }
}
