...
use super::{
    DEFINES, INCLUDES, INPUT, UNPUT, VARIABLES, YYLESS, YYLEX, YYMORE, YYWRAP, YY_ADD_BUFFER, YY_FATAL_ERROR, YY_IF_MATCH, YY_IF_NO_MATCH, YY_INCREASE_ACCEPTING_STACK_LEN, YY_INIT_ACCEPTING_STACK, YY_INIT_BUFFER, YY_NEXT_CHAR, YY_POP_ACCEPTING_STATE, YY_PUSH_ACCEPTING_STATE, YY_REJECT, YY_SET_YYTEXT
};
...
pub fn write_yywrap(file: &mut File) -> std::io::Result<()> {
    let file_content = open_template_file(YYWRAP)?;
    file.write_all(file_content.as_bytes())?;
    return Ok(());
}
